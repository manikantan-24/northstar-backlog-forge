...
- A Redis instance (optional, required for distributed state checkpointing and budget tracking)
...
Resources provisioned by `infra/terraform/`: Azure Container Registry, Container Apps, Key Vault (10 secrets via MSI), Azure Cache for Redis (Basic C0, atomic budget enforcement and LangGraph checkpoint persistence), Azure Files (logs/ 10 GB + outputs/ 50 GB), Log Analytics Workspace.
...
- **Distributed State Checkpointing** — Set `REDIS_URL` in your environment to automatically transition LangGraph state persistence from local memory (`MemorySaver`) to `RedisSaver`. This enables horizontally scaling container replicas to load and sync workflow states without session loss.
